...
class ResetPasswordApi(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        email_obj = EmailMessage('email/reset_password.tpl', {
            'token': token,
            'user': user
        }, 'parham-webdev@parham.com', to=[email])
...
class ConfirmResetPasswordApi(GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        return Response({'details': 'send new password via post request'}, status=status.HTTP_405_METHOD_NOT_ALLOWED)
    ...
    def post(self, request, token, *args, **kwargs):
        ...
        serializer = ConfirmResetPasswordSerializer(data=request.data, context={'user': user})
        # user sent to serializer for password validation
        serializer.is_valid(raise_exception=True)
        user.set_password(serializer.data.get('new_password'))
        user.save()
        return Response({'details': 'new password set to user successfully.'}, status=status.HTTP_200_OK)
