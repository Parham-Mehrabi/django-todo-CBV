...
class VerifyEmail(APIView):
    ...
    def get(self, request, token, *args, **kwargs):
        ...
        return Response({'details': 'your account has been verified successfully. '}, status=status.HTTP_202_ACCEPTED)
...
class ResendVerifyEmail(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        email_obj = EmailMessage('email/activation_email.tpl', {'token': token}, 'parham-webdev@parham.com', to=[email])
        EmailThread(email_obj).start()
        return Response({'details': 'user activation resend successfully'}, status.HTTP_200_OK)
    def get_token_for_user(self, user):     # noqa
        refresh = RefreshToken.for_user(user)
        return str(refresh.access_token)
